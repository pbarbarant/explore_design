...
reward_df = pd.concat([events, estimator.fit_predict(events)], axis=1)
diff = reward_df["IO__obsA"] - reward_df["IO__obsB"]
sign = np.where(reward_df["arm_choice"].eq("A"), 1, -1)

reward_df["Er"] = diff * sign
reward_df["Er"]
# %%
